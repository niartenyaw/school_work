...
enum ExpressionType { DOUBLE, TEXTSTRING, IDENTIFIER, COMPOUND, NULLEXP, RANGE };
...
public:
    RangeExp(std::string operation, std::string startCell, std::string endCell);
    virtual ~RangeExp();
    virtual double eval(EvaluationContext& context) const;
    virtual std::string toString() const;
    virtual ExpressionType getType() const;
private:
    Vector<double> getValues(EvaluationContext& context) const;
