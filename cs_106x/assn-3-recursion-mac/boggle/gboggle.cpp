...
void recordWordForPlayer(string word, Player player) {
   ...
   gwp->setFont(WORD_FONT + "-" + integerToString(WORD_FONT_SIZE));
   gwp->setColor(WORD_COLOR);
...
static void calculateGeometry(int numRows, int numCols) {
   ...
   gwp->setFont(WORD_FONT + "-" + integerToString(WORD_FONT_SIZE));
   gState.wordColumnWidth = gwp->getStringWidth("mmmmm");
   for (gState.fontSize = 8; gState.fontSize < 48; gState.fontSize++) {
      gwp->setFont(CUBE_FONT + "-" + integerToString(int(gState.fontSize)));
      if (gwp->getStringWidth("M") > FONT_PERCENTAGE*gState.cubeSize) break;
   }
}
...
static void drawCenteredChar(double xc, double yc, char ch) {
   string s(1, ch);
   gwp->setFont(CUBE_FONT + "-" + integerToString(int(gState.fontSize)));
   gwp->drawString(s, xc - gwp->getStringWidth(s)/2, yc + 0.4 * gState.fontSize);
}
...
static void fillRoundedRect(double x, double y, double width, double height,
   ...
   gwp->fillArc(x, y, 2 * r, 2 * r, 90, 90);
   gwp->fillArc(x, y + height - 2 * r, 2 * r, 2 * r, 180, 90);
   gwp->fillArc(x + width - 2 * r, y + height - 2 * r, 2 * r, 2 * r, 270, 90);
   gwp->fillArc(x + width - 2 * r, y, 2 * r, 2 * r, 0, 90);
}
...
static void drawRoundedRect (double x, double y, double width, double height,
   ...
   gwp->drawArc(x, y, 2 * r, 2 * r, 90, 90);
   gwp->drawArc(x, y + height - 2 * r, 2 * r, 2 * r, 180, 90);
   gwp->drawArc(x + width - 2 * r, y + height - 2 * r, 2 * r, 2 * r, 270, 90);
   gwp->drawArc(x + width - 2 * r, y, 2 * r, 2 * r, 0, 90);
}
