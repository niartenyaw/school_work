...
for i=1:128-42
    Fx2(i)=Fx1(i)+m(2).*x2DD(i);
    Fy2(i)=Fy1(i)+m(2)*g+m(2).*y2DD(i);
    T2(i)=T1(i)-Fy2(i)*r(2)*cos(theta2(i+41))...
        +Fx2(i)*r(2)*sin(theta2(i+41))...
        -Fy1(i)*(l(2)-r(2))*cos(theta2(i+41))...
        +Fx1(i)*(l(2)-r(2))*sin(theta2(i+41))+I(2)*t2DD(i+41);
    
    Fx3(i)=-m(4)*x4DD(i);
    Fy3(i)=m(4)*(y4DD(i)-g);
    T3(i)=Fy3(i)*r(4)*cos(theta4(i+41))-Fx3(i)*r(4)*sin(theta4(i+41))...
        -I(4)*t4DD(i+41);
end

figure;
plot(xx,T1/mass)
title('Torque in Ankle')
xlabel('Time (seconds)')
ylabel('Torque (Nm/kg)')

figure;
plot(xx,T2/mass)
title('Torque in Knee')
xlabel('Time (seconds)')
ylabel('Torque (Nm/kg)')
